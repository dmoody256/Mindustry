...
# env.Java(target= 'build', source=['repos/Arc/extensions/fx/src'])

# env.Append(JAVACFLAGS=[
#         '--add-exports', 'jdk.compiler/com.sun.tools.javac.model=ALL-UNNAMED',
#         '--add-exports', 'jdk.compiler/com.sun.tools.javac.processing=ALL-UNNAMED',  
#         '--add-exports', 'jdk.compiler/com.sun.tools.javac.tree=ALL-UNNAMED',  
#         '--add-exports', 'jdk.compiler/com.sun.tools.javac.util=ALL-UNNAMED', 
#         '--add-exports', 'jdk.compiler/com.sun.tools.javac.code=ALL-UNNAMED', 
#         '--add-exports', 'java.base/sun.reflect.annotation=ALL-UNNAMED', '-source', '1.8'])

# javapoet_jar = env.Command(
#     target='depends/javapoet.jar',
#     source=[javapoet_url],
#     action=getJarDependency
# )


# env.Append(JAVACLASSPATH=env.Glob('depends/*'))



# env.Depends(annotations_files, [javapoet_jar, arc_core])

#proc_files = env.Java(target='build', source=proc_files, JAVACLASSPATH=env['JAVACLASSPATH'] + ['repos/Mindustry/annotations/src/main/resources'], JAVACFLAGS=env['JAVACFLAGS'] + ['-proc:only'])
#env.Depends(proc_files, annotations_files)
#annotation_files = [file for file in glob.glob('repos/Mindustry/annotations/src/main/java/**/*.java', recursive=True) if os.path.splitext(os.path.basename(file))[0] not in annotations_test]
#process_files = [file for file in glob.glob('repos/Mindustry/annotations/src/main/java/**/*.java', recursive=True) if os.path.splitext(os.path.basename(file))[0] in annotations_test]
#print(str(annotations_test))
#print(str(annotation_files))
#print(str(process_files))
# annotations = env.Java(
#     target= 'build', 
#     source=annotation_files)

# env.Depends(annotations, arc_core)
# env.Depends(annotations, processor)

# process_annotations = env.Java(
#     target= 'build', 
#     source=process_files)
# env.Depends(process_annotations, annotations)




# env.Java(target= 'build', source=['repos/Mindustry/core/src'])
# env.Jar(target = 'desktop.jar', source = [ 'build' ])